...
- **Test Support**: Delegate injection APIs for Unit Test and Golden Test
...
**Unit Test / Golden Test Support**:

`StringsTestHelper` is an ARB-embedded helper generated by `monolith_localization`.
The test package's `pubspec.yaml` must depend on the output package (e.g. `foundation_resources`).
...
void main() {
  setUpAll(() async {
    // Example using StringsTestHelper generated by monolith_localization
  ...
  test('localized text is resolved from ARB', () {
    expect(LocalizeStringDelegate.get('some_message_id'), '期待する文言');
  });
}
```

Alternatively, you can pass ARB file contents via `readAsString()`.
Golden tests can still use `injectGoldenTestLocalization`.
